...
//        isTop: 1, // 是置顶按钮
    ...
    methods: {
      ...
      getList () {
        let that = this;
        contentService.getValues({pageNo: that.page.num, pageSize: that.page.size, name: that.query, type: 1}).then(function(res){
      ...
      add () {
        let that = this;
        that.$router.push({name: 'valueAdd'});
      },
      // 编辑
      doEdit (id) {
        let that = this;
        that.$route.params.valueId = id;
        that.$router.push({name: 'valueEdit'});
      },
      /*// 置顶
      toTop (id) {
        let that = this;
      ...
      handleOk () {
        ...
        contentService.deleteValue(id).then(function (res) {
          //console.log('删除', res);
          if(res.data.success){
            rows.splice(index, 1);
            that.page.num = 1;
